...
In this section, we assess the stationarity of the raw Bitcoin prices and their negative log returns, as stationarity is crucial for time series modeling.

We first plotted the Bitcoin prices (see Figure 1) to visually inspect for trends or patterns. The plot showed a clear trend, suggesting non-stationarity. To confirm this, we applied the Augmented Dickey-Fuller (ADF) test (see Table 1), which resulted in a p-value of 0.3885, indicating that the raw Bitcoin prices are non-stationary.

To address this, we computed the negative log returns, a transformation commonly used in financial time series analysis to obtain a stationary series. Visual inspection of the negative log returns (see Figure 2) suggested stationarity. This was further confirmed by the ADF test (see Table 2), which gave a p-value of 0.01, leading us to reject the null hypothesis of non-stationarity and confirming that the negative log returns are stationary.
...
ggplot(bitcoin_data_long, aes(x = Time, y = Values)) +
  ...
  labs(title = "Figure 1 and 2 : Bitcoin Prices and Negative Log Returns Over Time", 
       x = "Time", y = "Value") +
  theme_minimal()
```
...
Since the negative log returns deviate from normality, particularly in the extremes, we fit a t-distribution to the scaled data to check if it better captures these extreme values. A QQ-plot was generated to compare the negative log returns with the theoretical t-distribution (see Figure 5), which showed that the data fits the t-distribution quite well, including in the tails. For comparison, we also generated a QQ-plot for the normal distribution (see Figure 6), which demonstrated a poorer fit, particularly for extreme values. This suggests that the t-distribution, with its ability to model heavy tails, is a more appropriate fit for the data (see Figure 7).

Next, we compared the density plots of the normal and t-distributions. As expected, the t-distribution exhibited heavier tails than the normal distribution, meaning it better accounts for extreme values observed in the data (see Figure 8).
...
#### Figure 5: QQ-Plot of Negative Log Returns with t-Distribution
...
#### Figure 6: QQ-Plot of Negative Log Returns with Normal Distribution
