...
We first plotted the Bitcoin prices (see Figure 1) to visually inspect for any apparent trends or patterns. From the plot, we can already observe a clear trend and some repeating patterns, which suggest that the series may be non-stationary. However, to formally confirm this, we applied the Augmented Dickey-Fuller (ADF) test (see Table 1), which is a statistical test used to determine whether the series has a unit root—a key indicator of non-stationarity.
